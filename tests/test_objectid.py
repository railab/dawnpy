...
def _objectid_header_defs():
    return {
        ...
        "dtype": [
            {"value": 1, "type": "bool", "size": 1},
            {"value": 7, "type": "uint32", "size": 32},
            {"value": 15, "type": "block", "size": 0},
        ],
        "io_classes": {1: "dummy"},
        "proto_classes": {1: "serial"},
        "prog_classes": {1: "sampling"},
    }


@pytest.fixture
def source_free_headers(monkeypatch):
    bundle = type("ObjectIdHeaderBundle", (), {})()
    bundle.header_defs = _objectid_header_defs()
    monkeypatch.setattr(header_bundle, "load_header_bundle", lambda: bundle)
