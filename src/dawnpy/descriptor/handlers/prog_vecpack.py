...
yaml_type: str = "vecpack"
cpp_class: str = "CProgVecPack"


def config_fields() -> list[ConfigField]:  # pragma: no cover
    """Return the user-facing YAML config schema for ``vecpack``."""
    return [
        ConfigField(
            name="inputs",
...
def output_shape_owned_virt_targets(obj: Any) -> set[str]:
    """Return the vector output whose shape is owned by ``vecpack``."""
...
def validate_object_refs(obj: Any, io_map: dict[str, Any]) -> list[str]:
    """Validate ``vecpack`` input/output dtype constraints."""
    ...
    for input_ref in input_refs:
        ...
        if io.dtype != output.dtype:
            errors.append(
                f"Program {obj.obj_id} invalid: vecpack input "
...
def encode_binary(
    ...
    """Append ``vecpack``-specific config items to ``items``."""
    del decoder
