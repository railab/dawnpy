"""Handler for ``threshold`` PROG type."""
...
yaml_type: str = "threshold"
cpp_class: str = "CProgThreshold"


def config_fields() -> list[ConfigField]:
    """Return the user-facing YAML config schema for ``threshold``."""
    return [
...
def encode_binary(
    ...
    """Append ``threshold`` config items to ``items``."""
    del decoder
