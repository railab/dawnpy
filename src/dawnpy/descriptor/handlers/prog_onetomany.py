"""Handler for ``onetomany`` PROG type."""
...
yaml_type: str = "onetomany"
cpp_class: str = "CProgOneToMany"


def config_fields() -> list[ConfigField]:  # pragma: no cover
    """Return the user-facing YAML config schema for ``onetomany``."""
    return [
        ConfigField(
            name="input",
...
def validate_object_refs(obj: Any, io_map: dict[str, Any]) -> list[str]:
    ...
    for output_ref in output_refs:
        ...
        if io.dtype != input_io.dtype:
            errors.append(
                f"Program {obj.obj_id} invalid: onetomany output "
...
def encode_binary(
    ...
    """Append ``onetomany``-specific config items to ``items``."""
    del decoder
