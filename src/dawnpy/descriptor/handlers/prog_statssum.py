"""Handler for ``statssum`` PROG type."""
...
yaml_type: str = "statssum"
cpp_class: str = "CProgStatsSum"


def config_fields() -> list[ConfigField]:
    """Return the user-facing YAML config schema for ``statssum``."""
...
def encode_binary(
    ...
    """Append ``statssum`` config items to ``items``."""
    del decoder
