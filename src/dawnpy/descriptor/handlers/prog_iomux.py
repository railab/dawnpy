...
yaml_type: str = "iomux"
cpp_class: str = "CProgIOMux"


def config_fields() -> list[ConfigField]:  # pragma: no cover
    """Return the user-facing YAML config schema for ``iomux``."""
    return [
        ConfigField(
            name="control",
...
def output_shape_owned_virt_targets(obj: Any) -> set[str]:
    """Return the mux output whose shape is owned by the program."""
...
def validate_object_refs(obj: Any, io_map: dict[str, Any]) -> list[str]:
    ...
    for input_ref in input_refs:
        ...
        if io.dtype != output.dtype:
            errors.append(
                f"Program {obj.obj_id} invalid: iomux input "
...
def encode_binary(
    ...
    """Append ``iomux``-specific config items to ``items``."""
    del decoder
